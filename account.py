...
def open_account():
    ...
    while True:
        ...
        if len(username) <= 20:
            if username.isalnum():
                ...
                print("Username should contain only alphabets and digits"
                      " and no spaces or other characters")
        else:
    ...
    aadhaar_list = [i[4] for i in data.extract()]
    if aadhaar not in aadhaar_list:
        names_passwords = [(i[0], i[1]) for i in data.extract()]
